...
//outras
 vet:vetor;
 a,b,c,CT,CCT,np,ii,tot,tc,tt,ERR:integer;     //POSIV= INDICE DO VETOR
 num:string[2];
Begin
 assign(arq,'cand.ind');
 assign(TURMA,'TURMASFT.txt');
 reset(arq);
 reset(TURMA);
...
//fim da montagem
 tot:=ii;
 WRITELN(' PRIMEIROS 10 ELEMENTOS ANTES DA ORDENACAO. CC E PF');
 for a:=1 to 10 do
  writeln(' ',vet[a].cc,'  ',vet[a].pf:5);
 writeln(' inicio da ordenacao');
 ordem(vet,ii);
 writeln(' fim da ordenação');
 WRITELN(' PRIMEIROS 10 ELEMENTOS DO VETOR DEPOIS DA ORDENACAO. CC E PF');
 for a:=1 to 10 do
  writeln(' ',vet[a].cc,'  ',vet[a].pf:5);
